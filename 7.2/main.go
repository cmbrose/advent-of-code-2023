...
func getHandType(s string) handType {
	...
	// Accounts for when jokers are the multiple card
	countsOfCounts[0] = 1

	if countsOfCounts[5-js] == 1 {
		return handFiveOfAKind
	}
	if countsOfCounts[4-js] >= 1 {
		return handFourOfAKind
	}
	if (countsOfCounts[2] == 1 && countsOfCounts[3] == 1) ||
		(countsOfCounts[3] == 1 && js == 1) ||
		(countsOfCounts[2] == 2 && js == 1) ||
		(countsOfCounts[1] == 1 && countsOfCounts[2] == 1 && js == 2) ||
		(js == 3) {
		return handFullHouse
	}
	if countsOfCounts[3-js] >= 1 {
		return handThreeOfAKind
	}
	if countsOfCounts[2] == 2 || countsOfCounts[2] == 1 && js == 1 {
		return handTwoPair
	}
	if countsOfCounts[2-js] >= 1 {
		return handOnePair
	}
	return handHighCard
}
...
func main() {
	...
	total := 0
	for i, h := range hands {
		fmt.Printf("%s %s %d %d\n", h.hand.raw, util.SortStringChars(h.hand.raw), h.handType, h.bid)
		total += (i + 1) * h.bid
	}
